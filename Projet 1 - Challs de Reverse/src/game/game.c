...
static void append_event(char event_text[64],
                         const char *text)
{
    if (event_text[0] != '\0')
    {
...
static void build_move_event(Board *board,
                             const Move *move,
                             char event_text[64])
{
    event_text[0] =
...
static void print_game_header(void)
{
    ...
    printf(
        "Ton objectif : mettre l'ordinateur "
        "echec et mat le plus vite possible.\n"
    );


    printf(
        "Tu disposes de %d demi-coups maximum.\n\n",
        CHALLENGE_MAX_PLIES
    );
    ...
    printf(
        "  - Format incorrect : tu peux reessayer.\n"
    );


    printf(
        "  - Coup illegal     : la partie est arretee.\n\n"
    );
...
static int parse_move_input(const char *text,
                            int *from,
                            int *to)
{
    if (text == NULL ||
...
static void move_to_text(const Move *move,
                         char text[8])
{
    char from[3];
    char to[3];
    ...
    int *total_moves)
{
    ...
    Move actual_move;
    if (!challenge_read_ai_move(
            board,
            &actual_move
        ))
    {
        printf(
            "\nLe prochain coup de l'ordinateur "
            "est corrompu ou illegal.\n"
        );
        return 0;
    }
    PieceColor ai_color =
...
void game_start(void)
{
    ...
    if (!challenge_reset_move_file())
    {
        printf(
            "Impossible d'initialiser la partie.\n"
        );
    ...
    while (1)
    {
            ...
            ))
        {
            printf(
                "\nPat.\n"
                "Tu n'as pas battu l'ordinateur.\n"
            );
